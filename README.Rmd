...
where $H_{(Z,W)}$ is the hat/projection matrix for $(Z,W)$ and $D_{(Z,W)}$ is the diagonal matrix with diagonal elements corresponding to $H_{(Z,W)}$.
...
Note that $\hat{P}\_{IJIVE} = \hat{T}\_{IJIVE}$ because you have already residualized by $W$. 
